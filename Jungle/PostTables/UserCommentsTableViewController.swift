...
class UserCommentsTableViewController: PostsTableViewController {
    ...
    override func lightBoxVC() -> LightboxViewController {
        let vc = UserCommentsLightboxViewController()
    ...
    override func handleRefresh() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.35, execute: {
            SearchService.userComments(username: self.username, offset: self.state.posts.count) { posts in
