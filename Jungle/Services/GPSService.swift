...
final class LocationAPI:NSObject, CLLocationManagerDelegate {
    ...
    private(set) var region:Region? {
        didSet {
            NotificationCenter.default.post(name: Notification.Name.RegionLocationUpdated, object: self)
        }
    }
    ...
    private var lastSignificantLocation: CLLocation? {
        didSet {
            if let location = lastSignificantLocation {
                NotificationCenter.default.post(name: Notification.Name.GPSLocationUpdated, object: self)
                let params = [
