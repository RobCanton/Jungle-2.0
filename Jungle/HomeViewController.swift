...
class HomeViewController: UIViewController, NewPostsButtonDelegate {
    ...
    static func fetchData(state:State, lastPostID: Double?, completion: @escaping (_ posts:[Post], _ endReached:Bool)->()) {
        let postsRef = firestore.collection("posts").whereField("status", isEqualTo: "active").order(by: "createdAt", descending: true)
...
extension HomeViewController: ASTableDelegate, ASTableDataSource {
    ...
    func tableNode(_ tableNode: ASTableNode, willBeginBatchFetchWith context: ASBatchContext) {
        ...
        HomeViewController.fetchData(state: state, lastPostID: state.lastPostTimestamp) { posts, endReached in
            
            if endReached {
                let oldState = self.state
                self.state = HomeViewController.handleAction(.endReached(), fromState: oldState)
            }
            ...
            if self.state.isFirstLoad {
                let oldState = self.state
                self.state = HomeViewController.handleAction(.firstLoadComplete(), fromState: oldState)
                        ...
                        if success {
                            for i in 0..<self.state.posts.count {
                                ...
                                if arrayPost.key == post.key {
                                    self.state = HomeViewController.handleAction(.removePost(at: i), fromState: self.state)
